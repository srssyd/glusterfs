...
static void iob_release_buffer(iob_buffer_inode_t *inode){

}

static void iob_flush_buffer(call_frame_t *frame, xlator_t *this, fd_t *fd,iob_buffer_inode_t *inode_buffer){
...
static size_t insert_vector(call_frame_t *frame, xlator_t *this, fd_t *fd,iob_buffer_inode_t *inode_buffer, off_t offset,struct iovec *vector,uint32_t count,struct iobref *iobref){
    if(count <= 0)
        return 0;
    struct rb_table * vectors = inode_buffer->vectors;
           ...
           dict_t *xdata)
{
    ...
    LOCK(&buffer->lock);
    if((data=dict_get(buffer->buffer_dic,gfid))!=NULL){
